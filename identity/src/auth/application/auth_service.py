...
class AuthService:
    ...
    async def login(self, username: str, password: str) -> AccessToken:
        account: AccountWithPassword = await self.account_service.get_account_for_login(username)
        ...
        access_token = self.auth.generate_access_token(account.id, username, account.role)
        return access_token
