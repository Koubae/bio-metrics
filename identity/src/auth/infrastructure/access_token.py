...
class JWTAccessTokenGenerator(AccessTokenGenerator):
    ...
    def parse_access_token(self, access_token: str) -> AccessToken:
        settings = Settings.get()
        try:
            payload = jwt.decode(access_token, settings.get_cert_public(), algorithms=[self.JWT_ALGORITHM])
        except jwt.ExpiredSignatureError:
            logger.debug("Access-Token expired", extra={"extra": {"access_token": access_token}})
            raise AuthAccessTokenExpired("Token expired")
        except jwt.InvalidTokenError:
            logger.debug("Invalid Access-Token", extra={"extra": {"access_token": access_token}})
        ...
        try:
            ...
            logger.warning(f"Invalid Access-Token while parsing, error: {error}",
                         extra={"extra": {"access_token": access_token, "payload": payload, "error": repr(error)}})
