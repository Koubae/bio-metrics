...
UNEXPECTED_ERROR_MESSAGE: t.Final[str] = "Unexpected error occurred, please try again later!"
...
class RepositoryDatabaseConnectionError(RepositoryException):
    def __init__(self, model: type[DbModel], entity: Entity, error: str) -> None:
        super().__init__(f"Database connection Error {model} with values {entity}, error: {error}")
...
class RepositoryCreateException(RepositoryException):
    def __init__(
        ...
        message = message or f"Error while inserting {model} with values {entity}, error: {error}"
...
class RepositoryPKMissingException(RepositoryException):
    def __init__(self, model: type[DbModel], missing: str) -> None:
        super().__init__(f"Missing primary-key column for model {model}, missing: '{missing}'")
